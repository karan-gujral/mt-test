...
@Slf4j
public abstract class AbstractAsyncService<PT, ST> {

  private static InheritableThreadLocal<List<CompletionService<?>>> threadData = new InheritableThreadLocal<>();

  public List<CompletionService<?>> cache() {
    log.info("Cache called {}", threadData.get());
    if (threadData.get() == null || threadData.get().isEmpty()) {
      threadData.set(ImmutableList.<CompletionService<?>>of(
